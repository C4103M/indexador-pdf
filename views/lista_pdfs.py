...
class PdfListView:
    def __init__(self, page: Page, session: Session):
        ...
        self.btn_anterior = IconButton(
            Icons.NAVIGATE_BEFORE, on_click=self.ir_pagina_anterior
        )
        self.btn_proximo = IconButton(
            Icons.NAVIGATE_NEXT, on_click=self.ir_pagina_proxima
        )
    ...
    def carregar_dados(self):
        ...
        pdfs_da_pagina = self.pdf_repo.find_all(
            page=self.pagina_atual, per_page=self.itens_por_pagina
        )
        ...
        self.status_paginacao.value = (
            f"Página {self.pagina_atual} de {self.total_de_paginas}"
        )
